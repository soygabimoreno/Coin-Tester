...
public class AxisTickLabels {
    ...
    public static void draw(Canvas c, ScreenPhysicalMapping axisMap, GridLabel gridLabel,
        ...
        for (int k = 0; k < 2; k++) {
            ...
            for (int i = 0; i < values.length; i++) {
                posAlongAxis = (float) axisMap.pxFromValue(values[i]) * directionSign;
                if (drawOnXAxis) {
        ...
        // Straight line
        if (drawOnXAxis) {
            c.drawLine(axisBeginX, axisBeginY, axisBeginX + (float) axisMap.getNCanvasPx() * (1 - directionI), axisBeginY, labelPaint);
        } else {
            c.drawLine(axisBeginX, axisBeginY, axisBeginX, axisBeginY + (float) axisMap.getNCanvasPx() * (2 - directionI), labelPaint);
        }
        ...
        for (int i = 0; i < gridLabel.strings.length; i++) {
            posAlongAxis = (float) axisMap.pxFromValue(gridLabel.values[i]) * directionSign;
            ...
            while (j < gridLabel.strings.length) {
                float nextDigitPos = (float) axisMap.pxFromValue(gridLabel.values[j]) * directionSign;
