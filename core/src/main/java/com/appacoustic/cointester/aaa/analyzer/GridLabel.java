...
public class GridLabel {
    ...
    private static final String TAG = GridLabel.class.getSimpleName();

    // Never null!
    public double[] values = new double[0];  // TODO: use a better name?
    double[] ticks = new double[0];  // TODO: use a better name?

    public enum Type {  // java's enum type is inconvenient
    ...
    public GridLabel(Type _gridType, double _gridDensity) {
        gridType = _gridType;
    ...
    public void setGridType(GridLabel.Type gt) {
        gridType = gt;
    }

    public void setDensity(double _gridDensity) {
        gridDensity = _gridDensity;
    }
...
//        KLog.Companion.i("Note: " + startPitch + "(" + startValue + ") ~ " + endPitch + " (" + endValue + ") div " + gridPointsArray[0].length);
    ...
    public StringBuilder[] strings = new StringBuilder[0];
    ...
    public void updateGridLabels(double startValue, double endValue) {
